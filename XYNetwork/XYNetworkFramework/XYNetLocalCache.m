...
static NSString *  XYCacheFileProcessingQueue = @"com.eoc.SunyNetCache";
static NSString *  XYCacheDocument = @"XYCache";
static const NSInteger XYDefaultCacheMaxDeadline = 60 * 60 * 24 * 30; // 1 个月

@interface XYNetLocalCache (){
    NSCache          *_memoryCache;//cache 用来解决频繁从文件系统中 读取文件的 系能问题
    NSString         *_cachePath;
    NSFileManager    *_fileManager;
    dispatch_queue_t _XYIOQueue;
    NSMutableSet     *_protectCaches;
}

@end

@implementation XYNetLocalCache


+ (instancetype)sharedInstance
{
    static XYNetLocalCache *instance;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _XYIOQueue = dispatch_queue_create([XYCacheFileProcessingQueue UTF8String], DISPATCH_QUEUE_CONCURRENT);
        _maxCacheDeadline = XYDefaultCacheMaxDeadline;
        
        dispatch_sync(_XYIOQueue, ^{
            _fileManager = [NSFileManager new];
            NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
            _cachePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:XYCacheDocument];
            BOOL isDir;
